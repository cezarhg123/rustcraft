...
pub struct Camera {
    ...
    buffer_info: WriteDescriptorInfo,
    rotating: bool,
    accept_input: bool
}
...
impl Camera {
    ...
    pub fn new(position: glm::Vec3, vust: &mut Vust) -> Camera {
        ...
        let uniform_buffer = Buffer::builder()
            .with_name("Camera uniform buffer")
        ...
        Camera {
            ...
            projection,
            view,
            buffer_info: WriteDescriptorInfo::Buffer {
                buffer: uniform_buffer.handle(),
                offset: 0,
                range: size_of::<glm::Mat4>() as u64 * 2
            },
            uniform_buffer,
            rotating: false,
            accept_input: true
    ...
    pub fn inputs(&mut self, window: &mut Window, delta_time: f32) {
        ...
        self.view = glm::look_at_rh(&self.position, &(self.position + self.direction), &Self::UP);
